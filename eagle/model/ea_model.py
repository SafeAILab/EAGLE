...
class EaModel(nn.Module):
    ...
    def eagenerate_with_metrics(
        ...
        for idx in range(max_length):
            ...
            new_token_list.append(new_token - last_new_token)
        ...
        metrics['new_token_list'] = new_token_list
        metrics['accept_rate'] = sum(new_token_list) / (len(new_token_list) * self.ea_layer.depth) if new_token_list else 0

        if not log:
            return input_ids
        else:
            return input_ids, new_token, idx, metrics
