...
def bot(history, temperature, top_p, use_EaInfer, highlight_EaInfer):
    ...
    if use_EaInfer:
        ...
        for output_ids in model.ea_generate(input_ids, temperature=temperature, top_p=top_p,
            ...
            yield history,f"{new_tokens/totaltime:.2f} tokens/s",f"{new_tokens/total_ids:.2f}"
            start_time = time.time()
...
def regenerate(history):
    if not history:
        return history, None,"0.00 tokens/s"
    global pure_history
    ...
    if len(history) > 1:  # Check if there's more than one entry in history (i.e., at least one bot response)
        ...
        return new_history + [[last_user_message, None]], None
    history[-1][1] = None
    return history, None,"0.00 tokens/s"  # Return the original history if there's no bot response
...
with gr.Blocks(css=custom_css) as demo:
    ...
    note=gr.Markdown(show_label=False,interactive=False,value='''If "Highlight the tokens generated by EAGLE" is checked, the tokens correctly guessed by EAGLE 
    ...
    enter_event=msg.submit(user, [msg, chatbot], [msg, chatbot], queue=True).then(
        bot, [chatbot, temperature, top_p, use_EaInfer, highlight_EaInfer], [chatbot,speed_box,compression_box]
    )
    clear_button.click(clear, chatbot, [chatbot,speed_box,compression_box], queue=True)

    send_event=send_button.click(user, [msg, chatbot], [msg, chatbot],queue=True).then(
        bot, [chatbot, temperature, top_p, use_EaInfer, highlight_EaInfer], [chatbot,speed_box,compression_box]
    )
    regenerate_event=regenerate_button.click(regenerate, chatbot, [chatbot, msg,speed_box],queue=True).then(
        bot, [chatbot, temperature, top_p, use_EaInfer, highlight_EaInfer], [chatbot,speed_box,compression_box]
    )
