...
def _load_model(checkpoint_path, device="cuda", precision=torch.bfloat16):
    ...
    with torch.device('meta'):
        model = Transformer.from_config(str(path.parent/"config.json"))

    if "int8" in str(checkpoint_path):
        print("Using int8 weight-only quantization!")
        from .quantize_llama import WeightOnlyInt8QuantHandler
    ...
    if "int4" in str(checkpoint_path):
        ...
        from .quantize_llama import WeightOnlyInt4QuantHandler
