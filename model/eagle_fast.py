...
def _load_model(checkpoint_path, device="cuda", precision=torch.bfloat16,bias=True):
    ...
    with torch.device('meta'):
        model = EAGLE.from_config(str(path.parent/"config.json"),bias=bias)

    if "int8" in str(checkpoint_path):
        print("Using int8 weight-only quantization!")
        from .quantize_eagle import WeightOnlyInt8QuantHandler
    ...
    if "int4" in str(checkpoint_path):
        ...
        from .quantize_eagle import WeightOnlyInt4QuantHandler
...
class EAGLE(nn.Module):
    def __init__(self, config: ModelArgs,bias=True) -> None:
        super().__init__()
        self.config = config

        self.tok_embeddings = nn.Embedding(config.vocab_size, config.dim)
        if bias:
            self.fc=nn.Linear(config.dim*2,config.dim)
        else:
    ...
    def forward(self, hidden_states: Tensor,idx: Tensor, input_pos: Tensor,attn_pos:Tensor,mask:Tensor) -> Tuple[Tensor,Tensor]:
        assert self.freqs_cis is not None, "Caches must be initialized first"
        #mask = self.causal_mask[None, None, input_pos]
        freqs_cis = self.freqs_cis[input_pos]
        emb = self.tok_embeddings(idx)
        #self.emb=x
        fco = self.fc(torch.cat((emb, hidden_states), dim=-1))
        #self.fco=x


        x = self.layers[0](fco, attn_pos, freqs_cis, mask)
        # x = self.norm(x)
        logits = self.output(x)
        return logits,x

    @classmethod
    def from_config(cls, path: str,bias:Optional[bool]=True):
